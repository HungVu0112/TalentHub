...
const Jobs = () => {
    ...
    const getHumanJobCategory = (jobCategory) => {
        ...
        for (const category of JOB_CATEGORIES) {
             // Thêm kiểm tra category và value tồn tại
            ...
            <div className='jobs-container'>
                <div className='jobs-container__filter_tab'>
                    ...
                    <div className='dropdown'>
                        ...
                        <select data-testid="job-category-select" name="job_category" id="job_category" value={filterParams.job_category} onChange={handleInputChange}>
                            <option value="all">{intl.formatMessage(messages.all_job)}</option>
                ...
                <div className='jobs-container__search_tab'>
                    ...
                    <button data-testid="search-button" onClick={handleFilter}>
                        {intl.formatMessage(messages.search)}
                    </button>
                </div>

                <div className='jobs-container__job_list' data-testid="job-list-container">
                    {jobs.length === 0 ? (
                        <p className='empty_job' data-testid="empty-jobs-message">{intl.formatMessage(messages.empty)}</p>
                    ) : (
                        jobs.map(job => (
                            <JobCard
                                key={job.id} // Key đã có sẵn
                                jobData={{
                                    id: job.id,
                                    title: job.title,
